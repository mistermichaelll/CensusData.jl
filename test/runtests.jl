...
@testset "results of get_acs_data() make sense" begin
    ...
    dc = get_acs_data(; year=2021, survey="acs5", vars=vars, _for="place", _in="state:11")
    ...
    @test sum([eltype(dc[!, Symbol(col)]) == Int64 for col in vars]) == 4 # all these columns should be int64
end;
